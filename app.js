...
class Api {
  ...
  static async Requests(paths){
    try {
      const responses = await Promise.all(paths.map(path => fetch(`${this.API_URL}/${path}`)));
...
class Data {
  ...
  async loadGuildRanking(){
    ...
    const jsonDataList = await Api.Requests(group.map(worldId => `${worldId}/guild_ranking/latest`));
  ...
  async loadPlayerRanking(worldId, guildId){
    if (!this.guildWorldIdSet.has(worldId)){
      ...
      Object.assign(this.guildMap, players.reduce((groups, player) => {
...
class UI {
  ...
  createGuildCell(container, rank, worldId, guildName, guildId, bp, serverName, onSelect){
    ...
    if (ui.submitButtonLabel.text() == 'close'){
      ...
      const createGuildRanking = (container, startIndex) => {
        for (var index = startIndex; index < startIndex + 16; ++index){
          ...
          ui.createGuildCell(container, index + 1, v.world_id, v.name, v.id, v.bp, serverName, async () => {
            ...
            for (const player of playerBpList){
              ui.createPlayerCell(ui.playerRankingCellContainer, ++count, player.name, player.bp);
            }
